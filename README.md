...
- 主要ツールチェーンは**精密バージョン固定**で再現性を担保: `jest` は next / gas / add-app で `30.3.0` 固定（30.4.x の native-ESM `--experimental-vm-modules` 不具合を回避）、Expo のみ `jest-expo 57`（`jest 29.7.0`）。**現行版**: Next **16.2.10** / React **19.2.7**（Expo は React Native 整合で **19.2.3**）/ **Expo SDK 57**（React Native **0.86**）/ TypeScript **6.0** / esbuild **0.28** / clasp **3.x**（`package-lock.json` でロック）。**更新は公式 CLI で in-place**（`@next/codemod upgrade` / `expo install --fix` + `expo-doctor`）＝ JS+JSDoc 構造を保ったまま版整合。
